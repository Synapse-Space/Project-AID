...
app.post("/transcribe", async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res
        .status(500)
        .json({ status: "error", message: "ASSEMBLYAI_API_KEY is not set" });
    }
    ...
    if (!audioBase64) {
      return res
        .status(400)
        .json({ status: "error", message: "audioBase64 is required" });
    }
...
app.post("/stitch", async (req, res) => {
  try {
    ...
    if (clipPaths.length < clips.length) {
      console.log(
        `Warning: Only ${clipPaths.length} out of ${clips.length} clips were successfully downloaded`
      );
    }
    ...
    await new Promise((resolve, reject) => {
      ffmpeg()
        ...
        .outputOptions([
          "-c",
          "copy", // Copy since all clips are now normalized
          "-movflags",
          "+faststart",
        ])
        .on("end", () => {
    ...
    try {
      ...
      console.warn(
        "Warning: Could not clean up all temporary files:",
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="sign_video.mp4"'
    );
